...
$$y = b_1 \cdot (t-p) + b_2 \cdot (t-p)(s-s_{mean}) + e$$
...
$$y = \big(b_1 + b_2 \cdot (s-s_{mean})\big) \cdot (t-p) + e$$
...
the best this equation can do to define an individual is by using their $t$, $s$, and $p$ values. so treat the individual here as a configuration of these variables' values. if $T$ is hypothetically set to 1, then output $Y$ answers "what does this individual look like when treated." this is nothing but $Y_1$ for that individual. if $T$ is set to 0, output $Y$ answers "what does this individual look like at baseline", which is $Y_0$. so if $T=1$, $Y_1 = c \cdot (1-p)$. if $T=0$, $Y_0 = c \cdot (0-p)$.

$$Y_1 - Y_0 = \big(c \cdot (1-p)\big) - \big(c \cdot (0-p)\big) = (c - p \cdot c) - (-p \cdot c) = c - p \cdot c + p \cdot c = c$$
...
now look at this cool trick. we know that the coefficient is the linear approximation of treatment effect. we know that $b_1$ is the linear approximation of average treatment effect. $c = b_1 + b_2 \cdot (s-s_{mean})$. therefore,

$$\text{blp treatment effect} = \text{blp average treatment effect} + b_2 \cdot (s-s_{mean})$$
